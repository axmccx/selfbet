...
public class AlarmService extends Service {
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    ...
    if (mFlutterView.isApplicationRunning()) {
      if (callbackMethodChannel != null) {
        callbackMethodChannel.invokeMethod("firedWhileApplicationRunning", null);
      }
    } else {
      if (AlarmService.sPluginRegistrantCallback != null) {
        AlarmService.sPluginRegistrantCallback.registerWith(mFlutterView.getPluginRegistry());
      }
...
//        manager.setExact(clock, startMillis, pendingIntent);
        manager.setAlarmClock(
                new AlarmManager.AlarmClockInfo(startMillis, pendingIntent),
                pendingIntent
        );
      }
    } else {
